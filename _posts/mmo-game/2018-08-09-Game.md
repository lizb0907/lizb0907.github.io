...
### 2.屏蔽字校验采用DFA算法（DFA全称为：Deterministic Finite Automaton,即确定有穷自动机）
...
```
分析：分别将聊天屏蔽字和命名屏蔽字字符串转为字符串数组，调用load（）加载方法
...
public class DFATest
{
    ...
    {
        String[] keys = {"中国共产党", "中国人", "台毒"};
        util = new KeyWordsACFilter(IGNORE_CHARS, SUBSTITUTE_CHAR);
        util.initialize(keys);
    }

    private static boolean isContain(String sensitive)
    {
        return util.contain(sensitive);
    }
}
```
...
采用java正则表达式，引入java.util.regex.Pattern包
```java
